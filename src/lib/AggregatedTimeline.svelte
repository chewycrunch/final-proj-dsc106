<script lang="ts">
	...
	function computeAggregatedEvents(group: Patient[]): AggregatedEvent[] {
		...
		return eventDefs.map(({ label, key }) => {
			...
			if (
				...
				(filteredTimes.length < times.length * 0.1 && filteredTimes.length < 5)
			) {
				console.warn(
					`Outlier filtering removed all or most data for ${label}. Original: ${times.length}, Filtered: ${filteredTimes.length}`
				);
	...
	function renderTimeline() {
		...
		const paddedMin =
			domainMin >= 0 && domainMin < padding && overallMinMean >= 0 ? 0 : domainMin - padding;
		const paddedMax = domainMax + padding;

		const svg = d3.select(timelineDiv).append('svg').attr('width', width).attr('height', height);
		...
		const colorScale = d3.scaleOrdinal().domain(allEventLabels).range(mutedDistinctColors);
		...
		const legendItems = legend
			.selectAll<SVGGElement, { label: string; key: string }>('g')
			.data(eventDefs)
			.enter()
			.append('g')
		...
		legendItems.each(function (this: SVGGElement, d) {
			...
			g.append('circle').attr('r', 8).attr('cy', 0).attr('fill', colorScale(d.label));
			g.append('text')
				.attr('x', 16)
				.attr('y', 5)
				.attr('font-size', 13)
		...
		if (selectedDepartment)
			title += title ? `, Department: ${selectedDepartment}` : `Department: ${selectedDepartment}`;
...
<div>
	<div class="filter-controls" style="margin-bottom: 1.5rem;">
		...
		<select id="department" bind:value={selectedDepartment} style="margin:0 1rem 0.5rem 0;">
			<option value={null}>All</option>
			{#each Array.from(new Set(patients.map((p) => p.department))).filter(Boolean) as dept}
