<script lang="ts">
	...
	$: medianAge = filteredData.length
		? median(filteredData.filter((d) => d.age !== undefined && !isNaN(d.age)).map((d) => d.age)) ||
			0
		: 0;
	...
	function draw() {
		...
		g.append('g').call(axisLeft(y)).style('font-size', '12px');
		...
		if (ageRange[0] !== null && ageRange[1] !== null) {
			brushG.call(brush.move, [x(ageRange[0]), x(ageRange[1])]);
		}
	}
...
<div class="age-distribution">
	...
	<!-- <div class="insights">
		<p class="insight-text text-black">
			...
			{#if showBySex}
				<span class="hint">
					Up through the 40-to-50 age range, female cases slightly outnumber males—likely reflecting
					a higher volume of elective and gynecologic‐related procedures in middle-aged women. After
					age 60, male surgical cases begin to exceed female, which may be driven by increased
					cardiovascular and oncologic interventions in older men.
				</span>
			{/if}
		</p>
	</div> -->
